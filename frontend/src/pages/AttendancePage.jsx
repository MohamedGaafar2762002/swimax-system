          ...
          <table className="min-w-full divide-y divide-slate-700/50 text-sm">
            ...
            <tbody className="divide-y divide-slate-700/40">
              {summaryLoading ? (
                ...
                payroll.map((item) import { useCallback, useEffect, useMemo, useState } from "react";
import api from "../services/api.js";
import CoachesPagination from "../components/CoachesPagination.jsx";
import ConfirmModal from "../components/ConfirmModal.jsx";
import FullscreenModal from "../components/FullscreenModal.jsx"; // ✅ الجديد
import { formatDuration, hoursToMinutes } from "../utils/formatDuration.js";
...
export default function AttendancePage() {
  ...
  const loadReferences = useCallback(async () => {
    try {
      const [coachesRes, sessionsRes] = await Promise.all([
        api.get("/api/coaches", { params: { page: 1, limit: 200 } }),
        api.get("/api/sessions", { params: { page: 1, limit: 300 } }),
      ]);
